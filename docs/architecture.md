...
| `analyst` | `candidate_mix.large_cap_pct` / `crypto_pct` / `screener_pct` | fraction of `pool_size` drawn from each bucket (defaults 0.40/0.30/0.30); `crypto_pct`'s weight is silently redistributed to the other two buckets when `trading.enable_crypto` is false |
